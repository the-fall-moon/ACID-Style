...
class AutoencoderKLResi(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        if self.synthesis_data:
             gts, latents, _ = self.get_input_synthesis(batch, val=False)
        else:
             gts, latents, _ = self.get_input(batch)
        reconstructions, posterior = self(gts, latents)
