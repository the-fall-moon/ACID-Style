...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        ...
        if noise_schedule.schedule == 'discrete':
            return torch.round((t_continuous - 1. / noise_schedule.total_N) * 1000.)
        else:
            return t_continuous
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        # print(t_input)
        if cond is None:
            raise Error("you need the condition!")
            output = model.apply_model(x, t_input, **model_kwargs)
        else:
            t_cond = cond.copy()
            t_cond["c1"] = model.structcond_stage_model(t_cond["c1"], t_input)
            output = model.apply_model(x, t_input, t_cond, **model_kwargs)
        if model_type == "noise":
            return output
        # 以下都没用到
...
class DPM_Solver:
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        atol=0.0078, rtol=0.05,early_stop_step = None,train_mode = False,
    ):
        """
        ...
        if method == 'adaptive':
            ...
            # with torch.enable_grad():
            x_predx0_list = []
            x_predx0_list.append(x)
            if early_stop_step is not None and early_stop_step == 0:
                return x,x_predx0_list,timesteps
            vec_t = timesteps[0].expand((x.shape[0]))
            model_prev_list = [self.model_fn(x, vec_t)]
            x_predx0_list.append(model_prev_list[-1]) ###########
            t_prev_list = [vec_t]
            ...
            for init_order in range(1, order):
                ...
                if early_stop_step is not None and init_order == early_stop_step:
                    return x,x_predx0_list,timesteps
