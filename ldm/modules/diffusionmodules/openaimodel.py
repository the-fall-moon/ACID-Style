...
from ldm.modules.attention import SpatialTransformer, SpatialTransformerV2, SpatialTransformerV3_1
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, struct_cond=None, seg_cond=None, only_style=False, only_content=False):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformerV3_1) or \
                isinstance(layer, SpatialTransformerV2) or isinstance(layer, SpatialTransformer):
                assert context is not None or seg_cond is not None
                x = layer(x, context,seg_cond, only_content)
            elif isinstance(layer, ResBlockDual_origin):
                x = layer(x, emb)
            elif isinstance(layer, TimestepBlockDual):
                assert struct_cond is not None
                x = layer(x, emb, struct_cond, only_style)
...
class ResBlockDual(TimestepBlockDual):
    ...
    def forward(self, x, emb, s_cond, only_style=False):
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
        ...
        """
        return checkpoint(
            self._forward, (x, emb, s_cond, only_style), self.parameters(), self.use_checkpoint
        )


    def _forward(self, x, emb, s_cond, only_style=False):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
        ...
        if self.use_scale_shift_norm:
            ...
            h = self.out_layers(h)
        if not only_style:
            h = self.spade(h, s_cond) #这里
        return self.skip_connection(x) + h
    
class ResBlockDual_origin(TimestepBlockDual):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def forward(self, x, emb, *args, **kw):
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
...
class UNetModelDualtransferV4_1(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        semb_channels=None,
        style_dim=None,
        style_extra_context_tokens=4,
        style_tokens_scale=1.,
    ):
        super().__init__()
        if use_spatial_transformer:
            assert context_dim is not None, 'Fool!! You forgot to include the dimension of your cross-attention conditioning...'
        ...
        # 风格
        self.style_extra_context_tokens = style_extra_context_tokens
        self.style_dim = style_dim 
        self.context_dim = context_dim
        self.style_proj = torch.nn.Linear(style_dim , self.style_extra_context_tokens * context_dim)
        self.style_norm = torch.nn.LayerNorm(context_dim)
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                num_head_channels=dim_head,
                                use_new_attention_order=use_new_attention_order,
                            ) if not use_spatial_transformer else SpatialTransformerV3_1(
                                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_transformer,
                                use_checkpoint=use_checkpoint,emb_channel=time_embed_dim,
                                style_tokens_scale=style_tokens_scale,
                            )
                        )
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self._feature_size += ch
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SpatialTransformerV3_1(  # always uses a self-attn
                            ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
    ...
    def initialize_weights(self):
        ...
        #     nn.init.normal_(self.style_emb[0].weight, std=0.02)
        #     nn.init.normal_(self.style_emb[2].weight, std=0.02)
        # Initialize timestep embedding MLP:
        nn.init.normal_(self.time_embed[0].weight, std=0.02)
        nn.init.normal_(self.time_embed[2].weight, std=0.02)

        # copy style_attn2 layers:
        # for block in self.input_blocks:
        #     for module in block:
        #         if hasattr(module, 'style_attn2'):
        #             nn.init.constant_(module.adaLN_modulation[-1].weight, 0)
        #             nn.init.constant_(module.adaLN_modulation[-1].bias, 0)
        

    def forward(self, x, timesteps=None, context=None, struct_cond=None, seg_cond=None, only_style=False, only_content=False, y=None,**kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False) #固定时间编码
        emb = self.time_embed(t_emb)          #dim=time_emb_dim=4*model_channel   1280

        if self.num_classes is not None:                #None
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)

        # print(seg_cond.shape)
        seg_cond = self.style_proj(seg_cond).reshape(-1,self.style_extra_context_tokens,self.context_dim)
        seg_cond = self.style_norm(seg_cond)
        # print(seg_cond.shape,context.shape)

        h = x.type(self.dtype)            # noisy image
        for module in self.input_blocks:
            h = module(h, emb, context, struct_cond,seg_cond,only_style,only_content)
            hs.append(h)
        h = self.middle_block(h, emb, context, struct_cond,seg_cond,only_style,only_content)
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context, struct_cond,seg_cond,only_style,only_content)
        h = h.type(x.dtype)
