...
class SetupCallback(Callback):
    ...
    def on_keyboard_interrupt(self, trainer, pl_module): # on_exception
        if trainer.global_rank == 0:
            print("Summoning checkpoint.")
            ckpt_path = os.path.join(self.ckptdir, "last.ckpt")
            trainer.save_checkpoint(ckpt_path)

    def on_pretrain_routine_start(self, trainer, pl_module): # 换成on_train_start
        if trainer.global_rank == 0:
...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        total_image = list()
        total_image.append(images["content"])
        total_image.append(images["style"])
        total_image.append(images["sample"])
        total_image = torch.cat(total_image,dim=0)
        filename = "gs-{:06}_e-{:06}_b-{:06}.png".format(
            global_step,
            current_epoch,
            batch_idx)
        path = root/filename
        save_image(make_grid(total_image,nrow=images["content"].shape[0]),path)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            with torch.no_grad():
                images = pl_module.log_images(batch, max_image=self.max_images, split=split, **self.log_images_kwargs)
                for k in images:
...
if __name__ == "__main__":
    ...
    if opt.resume:
        ...
        torch.nn.Module.load_state_dict.__defaults__=(False,False)
    ...
    # model
    if opt.resume:
        config.model.params.resume_ckpt_path = None
    ...
    default_modelckpt_cfg = {
        ...
        "params": {
            "dirpath": ckptdir,
            "filename": "epoch={epoch}_step={step}_loss={train_loss:.4f}",
            "monitor": "train_loss",
    ...
    if version.parse(pl.__version__) >= version.parse('1.4.0'):
        default_callbacks_cfg.update({'checkpoints': modelckpt_cfg})
        default_callbacks_cfg.update({'checkpoints_epoch': modelckpt_epoch_cfg})
